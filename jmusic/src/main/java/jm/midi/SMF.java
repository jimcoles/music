...
public final class SMF implements JMC{
	...
	public void write(OutputStream os) 
		...
		//write all tracks
		Enumeration enumElem = trackList.elements();
	...
	public void print(){
		Enumeration enumElem = trackList.elements();
		while(enumElem.hasMoreElements()){
			Track track = (Track) enumElem.nextElement();
			track.print();
		}
	}
    	/**
	...
	private void writeTrackChunk(DataOutputStream odos, Track track)
		...
		Enumeration enumElem = track.getEvtList().elements();
		enumElem = track.getEvtList().elements();
		//At this stage Except that all events are NoteOn events
		while(enumElem.hasMoreElements()){
			Event evt = (Event) enumElem.nextElement();
			evt.write(dos);
			if(DEBUG)evt.print();
		}
