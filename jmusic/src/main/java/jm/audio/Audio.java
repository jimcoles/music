...
public final class Audio implements jm.JMC{
	...
	public static void processScore(Score score, Instrument[] instList, String fileName){
		...
		while(enumElem.hasMoreElements()){
			...
			if(part.getInstrument() != NO_INSTRUMENT){
				try{
					inst = instList[part.getInstrument()];
				}catch(ArrayIndexOutOfBoundsException npe){
					System.out.println("Sorry ... I can't find the instrument number "+
                                                part.getInstrument() + " that you have specified for "+
                                                "the part named " + part.getTitle()+".");
					System.exit(1);
				}
			}
                         System.out.print("Part "+ partCounter++ + " '"+part.getTitle()+"', ");
			/* Enumerate through all phrases */
			Enumeration enumElem2 = part.getPhraseList().elements();
                        
                        int phraseCounter = 0;
			while(enumElem2.hasMoreElements()){
                           
				Phrase phr = (Phrase) enumElem2.nextElement();
				//get phrase tempo
				...
				if(phr.getInstrument() != NO_INSTRUMENT){
					try{
						inst = instList[phr.getInstrument()];
					}catch(ArrayIndexOutOfBoundsException npe){
						System.out.println("Sorry ... I can't find the instrument number "+
				...
				Enumeration enumElem3 = phr.getNoteList().elements();
				...
				while(enumElem3.hasMoreElements()){
					...
					Note note = (Note) enumElem3.nextElement();
					...
					inst.renderNote(new_note,((double)time+ntime));
					inst.setFinished(false);
					inst.iterateChain();
					ntime += phrase_ratio * note.getRhythmValue();
				}
	...
	public static void combine(String fileJmp, String tmpFile, String fileOut, 
		...
		if(multi){
			...
			System.out.println("Sample rate: " + SampleOut.samprate +
	...
	public static void addEmUp(String tmpFile, String fileName, float max){
		if(VERBOSE) {
		...
		try{
			...
			int offset = 28;
			int numOfBytes = ((int)tmpF.length()/2)+16;
			int format = 3;//6; 
			//int channels = 2;
			//write header
