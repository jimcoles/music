 ...
 * The <bl>Compresor</bl> can take one Audio Object as input. 
* @author Andrew Brown
		...
		int returned = this.previous[0].nextWork(buffer);
                for(int i=0;i<returned;i++){
                    ...
                    if(buffer[i] > threshold) {
                        buffer[i]= threshold + (buffer[i] - threshold) / this.ratio;
                    }
                    // negative values
                    if(buffer[i] < threshold * -1.0f) {
                        buffer[i] = threshold * -1.0f + (buffer[i] - threshold) / this.ratio;
                    }
