...
public final class StereoPan extends AudioObject{
	 ...
	 * @param ao a single AudioObject input.
	 */
	public StereoPan(AudioObject ao, float pan){
		super(ao, "[StereoPan]");
		this.panSet = true;
		if(pan < (float)0.0){
			this.pan = (float)0.0;
	...
	public int work(float[] buffer)throws AOException{
		...
		// don't do anything for 1 channel data
		if(channels == 1) return returned;
		// change volume for stereo files
		for(int i=0;i<returned;i++){
			if(channel==1){
                 if(this.pan > 0.5) buffer[i] = (buffer[i] * (float)((1.0 - this.pan) * 2.0));      
                 channel=2;
			}else{
                  if(this.pan < 0.5) buffer[i] = (buffer[i] * (float)(this.pan * 2.0));
                  channel=1;
			}
		}
		return returned;
